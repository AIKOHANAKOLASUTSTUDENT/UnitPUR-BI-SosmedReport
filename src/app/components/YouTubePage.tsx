...
export default function YouTubePage() {
  const { platformAuth, youtubePosts, platformConnections, refreshBootstrap } =
    useOutletContext<OutletContextType>();

  const [urlInputs, setUrlInputs] = useState([
    { url: "", contentType: "Video" as YouTubePost["contentType"] },
  ]);
  ...
  const addUrlRow = () =>
    setUrlInputs([...urlInputs, { url: "", contentType: "Video" }]);
  ...
  const handleAnalyze = async () => {
    let validRows: Array<{
      url: string;
      contentType: YouTubePost["contentType"];
    }> = [];
    ...
    if (bulkMode) {
      ...
      validRows = urls.map((url) => ({
        url,
        contentType: "Video" as YouTubePost["contentType"],
      }));
    } else {
        ...
        >
          <CardContent sx={{ p: 5 }}>
            ...
            <Typography variant="body2" sx={{ color: "text.secondary", mb: 4 }}>
              Connect a Google account with YouTube access to retrieve real
              video metrics through the official YouTube Data API.
            </Typography>
            <Button
              variant="contained"
              size="large"
            ...
            >
              OAuth tokens are stored securely on the backend and refreshed
    ...
    <Box>
        ...
        <CardContent>
          ...
          >
            <Typography variant="h6" sx={{ fontWeight: 600 }}>
              Add YouTube Content to Analyze
            </Typography>
          ...
          <Typography variant="body2" sx={{ color: "text.secondary", mb: 3 }}>
            Analyze Videos or Shorts. YouTube metrics include Views, Watch Time
            (hours), Subscribers Gained, Likes, Comments, and Shares.
          </Typography>
              ...
              {urlInputs.map((row, i) => (
                ...
                >
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      updateRow(i, "contentType", e.target.value)
                    }
                    size="small"
                    sx={{ minWidth: 130 }}
                  >
      ...
      {youtubePosts.length > 0 ? (
                ...
                <TableBody>
                  {youtubePosts.map((post) => (
                    <TableRow key={post.id} hover>
                      ...
                      <TableCell align="right">
                        {post.watchTimeHours.toLocaleString()}
                      </TableCell>
                      <TableCell align="right">
                        {post.subscribersGained.toLocaleString()}
                      </TableCell>
                      <TableCell align="right">
        ...
        <Alert severity="info" icon={<Youtube size={20} />}>
          No YouTube content analyzed yet. Enter video URLs above and click
