...
export async function runCrudExamples() {
  ...
  const socialAccountByPlatform =
    await socialAccountsRepository.findSocialAccountByPlatform(
      user.id,
      "INSTAGRAM",
    );
  const updatedSocialAccount =
    await socialAccountsRepository.updateSocialAccount(socialAccount.id, {
  ...
  const updatedContent =
    await monitoredContentRepository.updateMonitoredContent(content.id, {
  ...
  const latestMetric =
    await engagementMetricsRepository.findLatestEngagementMetricByContentId(
      content.id,
    );
  const updatedMetric =
    await engagementMetricsRepository.updateEngagementMetric(metric.id, {
      comments: 15,
    });
  ...
  const reportsByUser = await reportExportsRepository.findReportExportsByUserId(
    user.id,
  );
  const updatedReport = await reportExportsRepository.updateReportExport(
    report.id,
    {
