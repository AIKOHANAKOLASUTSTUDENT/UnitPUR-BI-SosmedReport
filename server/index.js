...
app.get("/api/platforms/:platform/status", async (req, res) => {
  try {
    const platform = validatePlatform(req.params.platform);
...
app.post("/api/platforms/:platform/disconnect", async (req, res) => {
  try {
    const platform = validatePlatform(req.params.platform);
    await saveConnection(platform, null);
...
app.post("/api/platforms/:platform/analyze", async (req, res) => {
  try {
    ...
    await savePlatformData(platform, analyses);
    ...
    const status = message.includes("Invalid content URL detected")
      ? 400
    ...
    res.status(status).json({
      error: message,
      code: status === 401 ? "SESSION_EXPIRED" : "ANALYSIS_FAILED",
    });
  }
});

app.get("/api/platforms/:platform/data", async (req, res) => {
  try {
    const platform = validatePlatform(req.params.platform);
    res.json({ data: await listPlatformData(platform) });
  } catch (error) {
    res.status(400).json({
      error:
        error instanceof Error
          ? error.message
          : "Unable to fetch platform data.",
    });
  }
});

app.post("/api/platforms/:platform/export", async (req, res) => {
  try {
    const platform = validatePlatform(req.params.platform);
    const body = z
      .object({
    ...
    const data = await listPlatformData(platform);
    if (data.length === 0) {
  ...
  try {
    ...
    res.status(400).json({
      error:
        error instanceof Error
          ? error.message
          : "Unable to read platform config.",
    });
  }
});
