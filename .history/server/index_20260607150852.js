...
app.get("/api/platforms/:platform/connect/start", async (req, res) => {
  try {
          const existingConnection = getConnection(platform);
          const { connection, refreshed } = await refreshConnectionIfNeeded(platform, existingConnection);
          if (refreshed) {
            saveConnection(platform, connection);
          }
