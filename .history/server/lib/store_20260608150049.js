...
async function getSystemUser() {
  const existingUser =
    await usersRepository.findUserByEmail(DEFAULT_USER_EMAIL);
  if (existingUser) {
    return existingUser;
  }
    ...
    case "INSTAGRAM":
      return {
        ...
        thumbnail:
          payload.thumbnail ||
          payload.image ||
          "https://placehold.co/80x80?text=IG",
        ...
        totalEngagement:
          latestMetric?.totalEngagement ||
          computeTotalEngagement(latestMetric || {}),
      };
    case "TIKTOK":
      return {
        ...base,
        thumbnail:
          ...
          "https://placehold.co/80x80?text=TT",
...
export async function bootstrapState() {
  ...
  const socialAccounts =
    await socialAccountsRepository.findSocialAccountsByUserId(user.id);
  const monitoredContent =
    await monitoredContentRepository.findMonitoredContentByUserId(user.id);
  ...
  const groupedContent = monitoredContent.reduce(
    (accumulator, record) => {
...
export async function saveConnection(platform, connection) {
  ...
  if (!connection) {
    ...
    for (const account of matches) {
      await monitoredContentRepository.deleteMonitoredContentBySocialAccountId(
        account.id,
      );
  ...
  const accountId = String(
    connection.accountId || `${normalizedPlatform}-account`,
  );
...
export async function clearPlatformData(platform) {
  const user = await getSystemUser();
  const normalizedPlatform = normalizePlatform(platform);
  const accounts = await socialAccountsRepository.findSocialAccountsByUserId(
    user.id,
  );
...
export async function listPlatformData(platform) {
  const user = await getSystemUser();
  const normalizedPlatform = normalizePlatform(platform);
  const records = await monitoredContentRepository.findMonitoredContentByUserId(
    user.id,
  );
