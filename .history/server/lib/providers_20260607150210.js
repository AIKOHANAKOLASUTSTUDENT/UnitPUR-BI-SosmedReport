...
async function verifyMetaConnection(platform, connection) {
  ...
  const debugToken = await fetchJson(`https://graph.facebook.com/${version}/debug_token?input_token=${encodeURIComponent(connection.accessToken)}&access_token=${encodeURIComponent(`${appId}|${appSecret}`)}`);
...
async function fetchYouTubeMetrics(connection, rows) {
  return Promise.all(rows.map(async (row, index) => {
    ...
    const response = await fetchJson(`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&id=${encodeURIComponent(videoId)}&access_token=${encodeURIComponent(connection.accessToken)}`);
