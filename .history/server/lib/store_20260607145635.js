...
export function bootstrapState() {
  ...
  return {
    platformAuth: Object.fromEntries(
      Object.entries(store.connections).map(([key, value]) => [
        key,
        Boolean(value?.connected && value?.verified),
      ]),
    ),
    platformConnections: Object.fromEntries(
      Object.entries(store.connections).map(([key, value]) => [
        key,
        mapConnection(value),
      ]),
...
export function saveConnection(platform, connection) {
  const store = readStoreFile();
  store.connections[platform] = connection
    ? mapPersistedConnection(connection)
    : null;
