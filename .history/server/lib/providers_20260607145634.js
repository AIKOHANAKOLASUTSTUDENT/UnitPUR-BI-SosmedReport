...
async function exchangeMetaCode(platform, code, callbackUrl) {
  ...
  const tokenUrl = new URL(`https://graph.facebook.com/${META_VERSION}/oauth/access_token`);
  ...
  const scopes = tokenResponse.granted_scopes ? String(tokenResponse.granted_scopes).split(',').map((scope) => scope.trim()).filter(Boolean) : providerConfig[platform].scopes;
...
async function exchangeGoogleCode(code, callbackUrl) {
  ...
  return {
    ...
    expiresAt: tokenResponse.expires_in ? new Date(Date.now() + Number(tokenResponse.expires_in) * 1000).toISOString() : null,
    scopes: tokenResponse.scope ? String(tokenResponse.scope).split(' ').map((scope) => scope.trim()).filter(Boolean) : providerConfig.youtube.scopes,
  };
}
...
async function exchangeTikTokCode(code, callbackUrl) {
  ...
  const tokenResponse = await fetchJson('https://open.tiktokapis.com/v2/oauth/token/', {
    method: 'POST',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
      client_key: clientId,
  ...
  return {
    ...
    refreshToken: tokenResponse.refresh_token || '',
    expiresAt: tokenResponse.expires_in ? new Date(Date.now() + Number(tokenResponse.expires_in) * 1000).toISOString() : null,
    scopes: tokenResponse.scope ? String(tokenResponse.scope).split(',').map((scope) => scope.trim()).filter(Boolean) : providerConfig.tiktok.scopes,
  };
}
...
function tokenExpired(connection) {
  return Boolean(connection.expiresAt && new Date(connection.expiresAt).getTime() <= Date.now());
}

async function verifyMetaConnection(platform, connection) {
  const version = META_VERSION;
  const profile = await fetchJson(`https://graph.facebook.com/${version}/me?fields=id,name&access_token=${encodeURIComponent(connection.accessToken)}`);
  const debugToken = await fetchJson(`https://graph.facebook.com/${version}/debug_token?input_token=${encodeURIComponent(connection.accessToken)}&access_token=${encodeURIComponent(process.env.META_APP_ID}|${process.env.META_APP_SECRET || ''})`);
...
async function verifyYouTubeConnection(connection) {
  const profile = await fetchJson(`https://www.googleapis.com/oauth2/v3/userinfo?access_token=${encodeURIComponent(connection.accessToken)}`);
  return {
...
function computeTotalEngagement(row) {
  ...
  return metrics.likes + metrics.comments + metrics.shares + metrics.saves + metrics.views + metrics.reach;
}

async function fetchMetaLibrary(platform, connection) {
  const version = META_VERSION;
  if (platform === 'instagram') {
    const media = await fetchJson(`https://graph.facebook.com/${version}/me/media?fields=id,caption,media_type,media_product_type,permalink,thumbnail_url,like_count,comments_count,timestamp&access_token=${encodeURIComponent(connection.accessToken)}`);
    return media?.data || [];
  }

  const posts = await fetchJson(`https://graph.facebook.com/${version}/me/posts?fields=id,message,permalink_url,created_time,shares,reactions.summary(true).limit(0),comments.summary(true).limit(0)&access_token=${encodeURIComponent(connection.accessToken)}`);
  const videos = await fetchJson(`https://graph.facebook.com/${version}/me/videos?fields=id,description,permalink_url,created_time,views,shares,comments.summary(true).limit(0)&access_token=${encodeURIComponent(connection.accessToken)}`);
  const photos = await fetchJson(`https://graph.facebook.com/${version}/me/photos?fields=id,name,permalink_url,created_time,reactions.summary(true).limit(0),comments.summary(true).limit(0)&access_token=${encodeURIComponent(connection.accessToken)}`);
  return [
    ...(posts?.data || []),
    ...(videos?.data || []),
    ...(photos?.data || []),
...
async function fetchInstagramMetrics(connection, rows) {
  ...
  return rows.map((row, index) => {
    const match = library.find((entry) => String(entry.permalink || '').replace(/\/$/, '') === row.url.replace(/\/$/, ''));
    if (!match) {
    ...
    return {
      ...
      totalEngagement: computeTotalEngagement({ likes: match.like_count, comments: match.comments_count }),
    };
  });
}
...
async function fetchFacebookMetrics(connection, rows) {
  ...
  return rows.map((row, index) => {
    const match = library.find((entry) => String(entry.permalink_url || '').replace(/\/$/, '') === row.url.replace(/\/$/, ''));
    if (!match) {
      throw new Error('Unable to access analytics data for this account.');
    }
    ...
    return {
      id: `${match.id || index}-${Date.now()}`,
      contentType: match.story ? 'Story' : match.is_reel ? 'Reel' : match.is_video ? 'Video' : 'Post',
      link: row.url,
      thumbnail: match.full_picture || match.picture || `https://placehold.co/80x80?text=FB`,
      caption: match.message || match.description || 'Facebook content',
      likes: reactions,
      comments,
      shares,
      reactions,
      reach: total(match.insights?.data?.find((item) => String(item.name).includes('reach'))?.values?.[0]?.value),
      impressions: total(match.insights?.data?.find((item) => String(item.name).includes('impression'))?.values?.[0]?.value),
      analyzedAt: new Date().toISOString(),
      totalEngagement: computeTotalEngagement({ likes: reactions, comments, shares, reach: 0, views: 0, saves: 0 }),
    };
  });
}

async function fetchTikTokMetrics(connection, rows) {
  const response = await fetchJson('https://open.tiktokapis.com/v2/video/list/', {
    method: 'POST',
    headers: {
      authorization: `Bearer ${connection.accessToken}`,
      'content-type': 'application/json',
    },
    body: JSON.stringify({ fields: ['id', 'share_url', 'cover_image_url', 'title', 'like_count', 'comment_count', 'share_count', 'view_count'] }),
  });

  const videos = response?.data?.videos || response?.data?.items || [];
  return rows.map((row, index) => {
    const match = videos.find((entry) => String(entry.share_url || '').replace(/\/$/, '') === row.url.replace(/\/$/, ''));
    if (!match) {
      throw new Error('Unable to access analytics data for this account.');
    }
    ...
    return {
      ...
      analyzedAt: new Date().toISOString(),
      totalEngagement: computeTotalEngagement({ likes: match.like_count, comments: match.comment_count, shares: match.share_count, saves: match.favorite_count || 0, views: match.view_count }),
    };
  });
}
...
async function fetchYouTubeMetrics(connection, rows) {
  ...
  return Promise.all(rows.map(async (row, index) => {
    const videoIdMatch = row.url.match(/(?:v=|shorts\/|youtu\.be\/)([A-Za-z0-9_-]{6,})/);
    if (!videoIdMatch) {
      throw new Error('Invalid content URL detected.');
    }

    const videoId = videoIdMatch[1];
    const response = await fetchJson(`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&id=${encodeURIComponent(videoId)}&key=${encodeURIComponent(YOUTUBE_API_KEY)}`);
    ...
    const kind = row.contentType || (row.url.includes('/shorts/') ? 'Short' : 'Video');
    return {
      id: `${video.id || index}-${Date.now()}`,
      contentType: kind,
      link: row.url,
      thumbnail: video.snippet?.thumbnails?.default?.url || `https://placehold.co/80x80?text=YT`,
      ...
      subscribersGained: 0,
      analyzedAt: new Date().toISOString(),
      totalEngagement: computeTotalEngagement({ likes: statistics.likeCount, comments: statistics.commentCount, views: statistics.viewCount }),
    };
  }));
}
