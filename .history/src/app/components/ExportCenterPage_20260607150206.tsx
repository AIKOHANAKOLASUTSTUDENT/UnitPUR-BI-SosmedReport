...
export default function ExportCenterPage() {
  ...
  const handleExport = async (format: "csv" | "xlsx") => {
    if (data.length === 0) {
      toast.error(
        `No ${platform.label} data to export. Please analyze some content first.`,
      );
      return;
    }
    ...
    try {
      const blob = await exportPlatformData(
        platform.key,
        format,
        currentSelected,
      );
      ...
      toast.success(
        `Downloaded ${platform.label} data as ${format.toUpperCase()} (${data.length} records)`,
      );
    } catch (error) {
  ...
  const exportOptions = [
    {
      ...
      description: "Download as Excel spreadsheet (.xlsx)",
    ...
    {
      title: "Download CSV",
      description: "Export as CSV for any data tool",
      icon: Download,
      color: "#f59e0b",
      format: "csv" as const,
    },
  ...
  return (
    <Box>
      ...
      <Tabs
        ...
        sx={{
          mb: 3,
          "& .MuiTabs-indicator": { bgcolor: platforms[activeTab].color },
        }}
      >
        {platforms.map((p, i) => {
          const Icon = p.icon;
            ...
            <Tab
              ...
              label={
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <Icon
                    size={16}
                    color={activeTab === i ? p.color : undefined}
                  />
                  {p.label}
                  {count > 0 && (
                    <Chip
      ...
      {data.length === 0 ? (
        <Alert severity="warning" sx={{ mb: 3 }}>
          No {platform.label} data available. Go to the {platform.label} menu to
          connect your account and analyze content first.
        </Alert>
      ) : (
            ...
            <CardContent>
              ...
              >
                ...
                <Box>
                  ...
                  <Typography
                    variant="h5"
                    sx={{ fontWeight: 700, color: platform.color }}
                  >
                    {platform.label}
                  </Typography>
                </Box>
          ...
          <CardContent>
            ...
            <List dense>
              {platform.fields.map((field) => (
                <ListItem key={field} disablePadding>
                  ...
                  <ListItemText
                    primary={(platform.fieldLabels as any)[field]}
                  />
                </ListItem>
              ))}
            </List>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              ...
              <Button
                ...
                fullWidth
                onClick={() =>
                  setSelectedFields((p) => ({ ...p, [platform.key]: [] }))
                }
              >
                Clear
              </Button>
