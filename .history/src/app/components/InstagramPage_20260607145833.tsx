...
export default function InstagramPage() {
  ...
  const [urlInputs, setUrlInputs] = useState([{ url: '', contentType: 'Post' as InstagramPost['contentType'] }]);
  ...
  const updateUrlRow = (index: number, field: 'url' | 'contentType', value: string) => {
  ...
  const handleAnalyze = async () => {
    let validUrls: Array<{ url: string; contentType: InstagramPost['contentType'] }> = [];
    ...
    if (bulkMode) {
      ...
      validUrls = urls.map((url) => ({ url, contentType: 'Post' as InstagramPost['contentType'] }));
    } else {
  ...
  const totalEngagement = (post: InstagramPost) => calculateTotalEngagement(post);
  ...
  if (!platformAuth.instagram) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
        <Card sx={{ maxWidth: 420, width: '100%', textAlign: 'center', border: `2px solid ${PLATFORM_COLOR}20` }}>
          <CardContent sx={{ p: 5 }}>
            <Box sx={{ width: 80, height: 80, borderRadius: '20px', background: 'linear-gradient(135deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 3 }}>
              <Instagram size={40} color="white" />
            </Box>
            <Typography variant="h5" sx={{ fontWeight: 700, mb: 1 }}>Connect Instagram</Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary', mb: 4 }}>
              Connect an Instagram Business or Creator account to analyze real engagement metrics from the official Meta Graph API.
            </Typography>
            <Button
              variant="contained"
              size="large"
              ...
              disabled={connecting}
              sx={{ bgcolor: PLATFORM_COLOR, '&:hover': { bgcolor: '#c2185b' }, py: 1.5 }}
            >
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4 }}>
        <Box sx={{ width: 48, height: 48, borderRadius: '12px', background: 'linear-gradient(135deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Instagram size={28} color="white" />
        </Box>
        <Box sx={{ flex: 1 }}>
          <Typography variant="h5" sx={{ fontWeight: 700 }}>Instagram Analysis</Typography>
      ...
      <Card sx={{ mb: 3, border: `1px solid ${PLATFORM_COLOR}30`, bgcolor: PLATFORM_BG }}>
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
            <Typography variant="h6" sx={{ fontWeight: 600 }}>Add Content to Analyze</Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                size="small"
                variant={!bulkMode ? 'contained' : 'outlined'}
                onClick={() => setBulkMode(false)}
                sx={!bulkMode ? { bgcolor: PLATFORM_COLOR, '&:hover': { bgcolor: '#c2185b' } } : {}}
              >
                Single
              </Button>
              <Button
              ...
              {urlInputs.map((row, i) => (
                <Box key={i} sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'center' }}>
                  ...
                  <TextField
                    ...
                    onChange={(e) => updateUrlRow(i, 'contentType', e.target.value)}
                    size="small"
                    sx={{ minWidth: 130 }}
                  >
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 2 }}>
            ...
            <Button
              ...
              disabled={analyzing}
              sx={{ ml: 'auto', bgcolor: PLATFORM_COLOR, '&:hover': { bgcolor: '#c2185b' } }}
            >
      ...
      {instagramPosts.length > 0 ? (
                    ...
                    <TableRow key={post.id} hover>
                      ...
                      <TableCell>
                        <Chip label={post.contentType} size="small"
                          sx={{ bgcolor: post.contentType === 'Reel' ? '#fce7f3' : post.contentType === 'Story' ? '#fef3c7' : '#f3e8ff', color: post.contentType === 'Reel' ? '#9d174d' : post.contentType === 'Story' ? '#92400e' : '#6b21a8' }} />
                      </TableCell>
        ...
        <Alert severity="info" icon={<Instagram size={20} />}>
          No Instagram content analyzed yet. Enter URLs above and click Analyze to get started.
        </Alert>
      )}
    </Box>
  );
